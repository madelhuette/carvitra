...
`
  ...
  static async extractVehicleData(
    ...
    if (!pdfText || pdfText.trim().length < 50) {
      console.warn('[AI Extraction] Text too short or empty:', pdfText?.length || 0, 'characters')
      return {
        vehicle: {},
        leasing: {},
        dealer: {},
    ...
    console.log('[AI Extraction] Text length:', pdfText.length, 'characters')
    if (pdfText.length > maxTextLength) {
      console.log('[AI Extraction] Text truncated to', maxTextLength, 'characters')
    }
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(`[AI Extraction] Calling Claude API (Attempt ${attempt}/${maxRetries})...`)
        ...
        console.log('[AI Extraction] Claude API response received')
        console.log('[AI Extraction] Token usage:', response.usage)
        ...
        if (!jsonMatch) {
          console.error('[AI Extraction] No JSON found in response:', content.text.substring(0, 500))
          throw new Error('No JSON found in Claude response')
        }

        console.log('[AI Extraction] Parsing JSON response...')
        ...
        console.log('[AI Extraction] Extracted vehicle:', result.vehicle?.make, result.vehicle?.model)
        console.log('[AI Extraction] Confidence score:', result.metadata.confidence_score)
        return result
        ...
        if (error.name === 'AbortError' || error.code === 'ECONNRESET' || error.code === 'ETIMEDOUT') {
          if (attempt < maxRetries) {
            ...
            console.log(`[AI Extraction] Waiting ${waitTime}ms before retry...`)
        ...
        if (error.status === 401) {
          console.error('[AI Extraction] API Key authentication failed - check ANTHROPIC_API_KEY')
          break
        }
    ...
    console.error('[AI Extraction] All attempts failed:', lastError)
    console.error('[AI Extraction] Error type:', lastError?.constructor.name)
    console.error('[AI Extraction] Error message:', lastError?.message)
