-- ============================================================================
-- Migration: Fix RLS Policies - Final Version
-- ============================================================================
-- Corrects all table relationships and RLS policies
-- ============================================================================
...
-- Drop all landingpages policies  
...
-- LANDINGPAGES POLICIES (via offer_id -> offers.organization_id)
CREATE POLICY "landingpages_select"
    ...
    USING (
        EXISTS (
            ...
            WHERE offers.id = landingpages.offer_id 
            AND user_has_organization_access(offers.organization_id)
        )
    );

-- Public access for published landing pages
CREATE POLICY "landingpages_public_select"
    ON landingpages FOR SELECT
    TO anon
...
CREATE POLICY "landingpages_delete"
    ...
    USING (
        EXISTS (
            SELECT 1 FROM offers 
            WHERE offers.id = landingpages.offer_id 
            AND user_is_organization_admin(offers.organization_id)
        )
    );

-- LEADS POLICIES (via landingpage_id -> offer_id -> organization_id)
CREATE POLICY "leads_select"
    ...
    USING (
        EXISTS (
            SELECT 1 
            FROM landingpages 
            JOIN offers ON offers.id = landingpages.offer_id
...
-- ============================================================================
-- STEP 4: Ensure handle_new_user function works correctly
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_landingpages_offer_id ON landingpages(offer_id);
...
CREATE INDEX IF NOT EXISTS idx_landingpages_offer_published ON landingpages(offer_id, published);
...
GRANT EXECUTE ON FUNCTION user_has_organization_access(UUID) TO anon;

-- ============================================================================
-- STEP 7: Verify all tables have RLS enabled
-- ============================================================================
